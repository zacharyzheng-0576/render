...
3. **导出功能**
   ...
   - PDF 会导出当前筛选下全部可见统计图表、图表统计明细、全部原始记录总览、完整字段附录
   - PDF 页面先以 HTML 排版并截图写入，避免 jsPDF 直接写中文导致乱码
   - 总表导出自动命名：`Crosspilot AI数据调研统计_日期_编号`，CSV / Excel / PDF 独立编号
   - 每个统计图表可单独导出 PDF，自动命名：`图表名称_日期_编号`，不同图表独立编号
...
1. **语言切换**：部分动态内容可能未完全翻译
2. **PDF 导出**：代码层已完成重构；因后台需要管理员密码，仍建议登录线上后台后手动下载一次完整 PDF 做最终人工确认
...
1. **测试问卷流程**
   - 测试所有分支路径
   - 测试浅度→深度的过渡
   - 登录后台后手动导出 PDF，检查下载文件页数、图表和字段附录
